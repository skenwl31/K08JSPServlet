...
속성 정리
 ...
 -->
	...
	<!-- 
	if(number%2==0)과 동일한 조건의 if문으로 해당 조건의
	결과가 result에 저장된다. 나머지는 0이므로 true가 저장된다.
	-->
	...
	eq를 통해 값에 대한 비교와 문자열에 대한 비교
	모두를 할 수 있다. 여기서 사용한 eq는 Java의 compareTo()와
	같이 사전순으로 문자열을 비교한다. -->
	...
	result4 : ${ result4 }<br>
	<!-- test에는 앞뒤에 하나라도 공백이 들어가면 무조건 
	false를 반환하게된다. 따라서 주의해야 한다. -->
	...
	<h4>연습문제 : if태그 </h4>
	<!-- 아이디 패스워드를 입력받은 후 submit버튼을 누르면
	EL식을 통해 파라미터를 받은후 tjoeun/1234인 경우에는
	...
	<c:if test="${ not empty param.user }">
		...
		<!--  아이디, 패스워드를 입력했을때만 일치여부를
		판단한 후 결과를 변수에 저장한다. 
		if태그는 else구문이 따로 없으므로 not을 통해 두번째
		if문을 구성해야한다.
		
		-->
		<c:if test="${ param.user== 'tjoeun' and param.pass eq '1234' }"
			var="loginResult">
			${ param.user }님, 하이루~_~<br>
		</c:if>
	...
	<c:if test="${ id eq tjoeun } && ${ pass eq 1234 }" var="result1">
		${ param.user }님, 하이룽~
		</c:if>
		result1 : ${ result1 }
	--%>
