...
public class ListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		*/
		...
		//전체 게시물의 갯수를 카운트하여 반환한다. 게시물의
		//갯수를 통해 가상번호를 부여하거나 전체페이지수를
		//계산한다.
		...
		/* 페이징처리 start */
		/*
		서블릿에서 application내장객체를 얻어온 후 web.xml에 
		설정한 컨텍스트 초기화 파라미터를 읽어 온다. 한페이지에
		출력할 게시물의 갯수와 한블럭당 출력할 페이지번호의 
		갯수이다.
		 */
		...
		/*
		목록에 첫 진입시에는 무조건 1페이지로 가정한 후 
		게시물을 얻어온다. 특정 페이지로 진입한 경우에는
		파라미터를 받아서 구간을 계산한다.
		*/
		int pageNum = 1;
		...
		//페이지 번호를 생성하기 위한 유틸리티 클래스의
		//메서드 호출
		//모델1방식의 게시판에서 사용했던 메서드를 그대로 사용.
