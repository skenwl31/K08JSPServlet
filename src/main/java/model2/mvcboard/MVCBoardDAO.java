...
public class MVCBoardDAO extends DBConnPool{
	...
	public MVCBoardDTO selectView(String idx) {
		...
		try {
			...
			rs = psmt.executeQuery();
			//하나의 게시물 이므로 if문을 통해 next()함수를 실행한다.
	...
	//패스워드 검증을 위한 메서드로 조건에 맞는 게시물을 
	//카운트 한다.
	public boolean confirmPassword(String pass, String idx) {
		boolean isCorr = true;
		try {
			//일련번호와 패스워드가 일치하는 게시물이 있는지
			//확인한다.
			...
			rs = psmt.executeQuery();
			//count()함수의 경우 조건에 맞는 레코드가 없으면 0을
			//반환하므로 어떤 경우에라도 결과값이 있다. 따라서
			//next()를 단독으로 실행한다.
			rs.next();
			if (rs.getInt(1) == 0) {
				isCorr = false;
